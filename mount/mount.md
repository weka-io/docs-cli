```
Usage:
mount <source> <target> [--option option] [--type type] [--no-mtab] [--sloppy] [--fake] [--verbose]
...
Options:
   ...
   -n, --no-mtab   Mount without writing in /etc/mtab. This is necessary for example when /etc is on a read-only
                   filesystem
